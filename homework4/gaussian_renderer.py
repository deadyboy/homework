...
class GaussianRenderer(nn.Module):
    ...
    def compute_gaussian_values(
        ...
        normalizer = 1.0 / (2.0 * torch.pi * torch.sqrt(det)).view(N, 1, 1)
        gaussian = normalizer * torch.exp(-0.5 * mahalanobis) ## (N, H, W)
    ...
    def forward(
        ...
        # 7. Compute weights
        one_minus_alpha = (1.0 - alphas).clamp(min=1e-6)
        transmittance = torch.cumprod(
            torch.cat([torch.ones_like(one_minus_alpha[:1]), one_minus_alpha], dim=0),
            dim=0
        )[:-1]
